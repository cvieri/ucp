var RecordingsC = UCPMC.extend({
	...
	display: function(event) {
		...
		$(".recording-header th[class!=\"noclick\"]").click( function() {
			...
			$.pjax({ url: "?display=dashboard&mod=recordings" + uadd + view + id, container: "#dashboard-content" });
		});
		...
		$("#delete_recording").click(function(e) {
			e.preventDefault();
			var id = $("#id").val();
			if (confirm(_("Are you sure you want to delete this recording?"))) {
				$("form input").prop("disabled", true);
				$("#delete_recording").text(_("Removing..."));
				$("#delete_recording").prop("disabled", true);
				$.post( "?quietmode=1&module=recordings&command=deleterecording", { id: id }, function( data ) {
					if (data.status) {
						$.pjax({
							url: "?display=dashboard&mod=recordings",
			...
			e.preventDefault();
			var recording = { numbers: [] }, $this = this;
			$("form input,textarea").not(".special").each(function(i, v) {
				var item = $(v);
                                if(item.prop("id")=='fcode')
                                {
                                    var FcodeValue = '0';
                                ...
                                {
                                    recording[item.prop("id")] = item.val();
                                }
			});
			$("form input").prop("disabled", true);
			...
			$.post( "?quietmode=1&module=recordings&command=updaterecording", { recording: recording }, function( data ) {			
                            if (data.status) {
                                $.pjax({
                                        url: "?display=dashboard&mod=recordings",
                                        container: "#dashboard-content"
                ...
                $("#save_button").click(function(e) {
                    ...
                    if($.trim($("#rname").val())=='')
                    {
                        alert('Please enter recording name');
                        $("#rname").focus();
                        return false;
                    }
                    ...
                    $(this).prop("disabled", true);
                    $.post( "?quietmode=1&module=recordings&command=addrecording", { recording: recording }, function( data ) {			
                        if (data.status) {
                            $.pjax({
                                    url: "?display=dashboard&mod=recordings",
                                    container: "#dashboard-content"
		...
		/*$(".recording-item").click(function() {
			$.pjax({
				url: "?display=dashboard&mod=recordings&view=editrecording&id=" + $(this).data("recording"),
				container: "#dashboard-content"
			});
		});*/
	...
	search: function(text) {
		...
		if (text !== "") {
			$.pjax({
				url: "?display=dashboard&mod=recordings&search=" + encodeURIComponent(text) + view + id,
				container: "#dashboard-content"
			});
		} else {
			$.pjax({
				url: "?display=dashboard&mod=recordings" + view + id,
				container: "#dashboard-content"
			});
		}
	},
