...
switch($_POST['type']) {
	case 'updatesession':
		$sipstation->saveFreeTrialSession($_POST['session']);
		break;
	case 'freetrial':
		$ft = $sipstation->setupFreeTrial($_POST['session']);

		if ($ft === false) {
			$json['status'] = $false;
		}
		$json['status'] = true;
		$json['status_message'] = $ft;
		break;
	case "editaccount":
		if(isset($_POST['key'])) {
			$data1 = array(
			...
			$key = $_POST['key'];
			try{
				$json_data = $pest->post('/2/e911/'.$did.'/'.$key,$data1,array('CLIENT_TYPE' => 'ssmodule-'.$mi['sipstation']['version']));
			} catch (Exception $e) {
			...
			if(!empty($_POST['global_failover']) && $_POST['global_failover'] != $_POST['global_failover_old']) {
				...
				try{
					$json_data = $pest->post('/2/failover/'.$key,$data1,array('CLIENT_TYPE' => 'ssmodule-'.$mi['sipstation']['version']));
				} catch (Exception $e) {
					$d = json_decode($e->getMessage(),true);
					$json['status'] = false;
					$json['status_message'] = _('Remote Server Error. Please try again in 10 minutes, If you continue to have problems please contact support and give them this error code') . ':(' . $d['message'] . ')';
	...
	case "editdid":
		...
		foreach($did_list as $did) {
			...
			if(!empty($did['failover']) && $did['failover'] != $did['failover_old']) {
				...
				$key = $_POST['key'];
				try{
					$json_data = $pest->post('/2/failover/'.$did['did'].'/'.$key,$data1,array('CLIENT_TYPE' => 'ssmodule-'.$mi['sipstation']['version']));
				} catch (Exception $e) {
					$d = json_decode($e->getMessage(),true);
					$json['status_message'] = _('Remote Server Error. Please try again in 10 minutes, If you continue to have problems please contact support and give them this error code') . ':(' . $d['message'] . ')';
				}
				...
				$key = $_POST['key'];
				try{
					$json_data = $pest->delete('/2/failover/'.$did['did'].'/'.$key,array('CLIENT_TYPE' => 'ssmodule-'.$mi['sipstation']['version']));
				} catch (Exception $e) {
					$d = json_decode($e->getMessage(),true);
					$json['status_message'] = _('Remote Server Error. Please try again in 10 minutes, If you continue to have problems please contact support and give them this error code') . ':(' . $d['message'] . ')';
				}
	...
	case "updatee911":
		...
		switch($_POST['stype']) {
			...
			case 'update':
				...
				try {
					$thing = $pest->post('/2/e911/'.$_POST['didmaster'].'/'.$_POST['key'],$array,array('CLIENT_TYPE' => 'ssmodule-'.$mi['sipstation']['version']));
				} catch (Exception $e) {
					$json['status'] = false;
			...
			case 'delete':
				try {
					$thing = $pest->delete('/2/e911/'.$_POST['didmaster'].'/'.$_POST['key'],array('CLIENT_TYPE' => 'ssmodule-'.$mi['sipstation']['version']));
				} catch (Exception $e) {
					$json['status'] = false;
					$json['status_message'] = _('Remote Server Error. Please try again in 10 minutes. If you continue to have problems please contact support and give them this error code').':('.$e->getMessage().')';
					break;
